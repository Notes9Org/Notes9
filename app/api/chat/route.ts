...
export async function POST(req: Request) {
  ...
  if (!supabaseToken || !NOTES9_API_BASE) {
    console.error('Notes9 chat is not configured: missing Supabase token or NOTES9_API_BASE');
    return new Response(
      JSON.stringify({
        error: 'Chat service not configured. A Supabase token and NOTES9_API_BASE are required.',
      }),

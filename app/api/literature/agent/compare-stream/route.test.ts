...
vi.mock('@/lib/catalyst-client', () => ({
  compareAgentStreamUrl: () => 'https://ai.example.com/paper-analyzer/stream',
}));
...
function req(body: unknown, token = 'caller-jwt') {
  return new Request('https://notes9.test/api/literature/agent/compare-stream', {
    method: 'POST',
...
describe('POST /api/literature/agent/compare-stream — ownership pre-validation', () => {

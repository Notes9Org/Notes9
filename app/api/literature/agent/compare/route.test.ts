 ...
 * reaches the upstream `fetch`, while a request made only of owned ids (or
 * no ids at all — this route allows an empty list) is forwarded unchanged.
 */
...
vi.mock('@/lib/catalyst-client', () => ({
  compareAgentBaseUrl: () => 'https://ai.example.com/paper-analyzer',
}));
...
function req(body: unknown, token = 'caller-jwt') {
  return new Request('https://notes9.test/api/literature/agent/compare', {
    method: 'POST',
...
beforeEach(() => {
  ...
  global.fetch = vi.fn(async () => ({
    ok: true,
    status: 200,
    json: async () => ({ role: 'assistant', content: 'ok' }),
  })) as unknown as typeof fetch;
});

describe('POST /api/literature/agent/compare — ownership pre-validation', () => {
  ...
  it('preserves existing behavior: an empty literature_review_ids list skips the ownership check and forwards', async () => {

...
const UPSTREAM = process.env.LITERATURE_BIOMNI_AGENT_URL?.replace(/\/$/, '') ?? '';
...
export async function POST(req: Request) {
  ...
  if (!UPSTREAM) {
    return NextResponse.json({
      role: 'assistant',
      content:
        'Biomni research-design agent is not connected yet. Set **LITERATURE_BIOMNI_AGENT_URL** to your HTTP endpoint. Request includes `mode: "research_design"` plus `literature_review_ids[]`.',
      answer:
        'Biomni research-design agent is not connected yet. Set LITERATURE_BIOMNI_AGENT_URL to your HTTP endpoint.',
    });
  }

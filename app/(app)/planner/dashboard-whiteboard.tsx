...
import { Shapes, Type, Trash2, X, Sparkles } from "lucide-react"
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function DashboardWhiteboard({
  ...
  return (
    <article className="flex h-full min-h-0 flex-col overflow-hidden rounded-[calc(var(--radius)+4px)] border border-border bg-card">
            ...
            >
              ...
              {editingId === n.id ? (
                <textarea
                  ...
                  onMouseDown={(e) => e.stopPropagation()}
                  className="mt-1 w-full resize-none bg-transparent text-[12.5px] leading-[17px] outline-none text-foreground"
                  rows={3}
                />
              ) : (
                <div className="mt-1 whitespace-pre-wrap text-[12.5px] leading-[17px] text-foreground">
                  {n.body || (

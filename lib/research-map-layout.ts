...
export function layoutResearchMap(
  ...
  // Large `nodesep` is the key. With many sibling children at the same
  // dagre rank (e.g. one project with 30 lab notes), tight spacing forces
  // them into a wall against the parent and every edge collides on one
  // handle. `network-simplex` produces a cleaner sibling distribution than
  // `tight-tree` for hub-and-spoke graphs.
  g.setGraph({
    rankdir: direction,
    nodesep: 90,
    ranksep: 220,

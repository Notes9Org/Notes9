...
export function RightSidebar() {
      ...
      try {
        ...
        if (data.citations && data.citations.length > 0) {
          ...
          data.citations.forEach((citation: any, index: number) => {
            ...
            let route = '';
            switch (sourceType) {
              case 'literature_review': route = `/literature-reviews/${sourceId}`; break;
              case 'protocol': route = `/protocols/${sourceId}`; break;
              case 'lab_note': case 'report': default: route = '';
            }
  ...
  return (
    <div className={cn(
      ...
      isExpanded ? "fixed inset-0 z-50 w-full h-full transition-none" : "h-full w-full min-w-0 transition-none"
    )}>
      {/* Hidden File Input */}

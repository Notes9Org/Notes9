...
| Literature normal (paper-analyzer SSE) | Next: `POST /api/literature/agent/compare-stream` → Notes9 `POST /paper-analyzer/stream` (same body as `/paper-analyzer`) | `query`, `literature_review_ids`, `session_id`, `history`, `options` | SSE: same family as Biomni stream (`thinking`, `token`, `clarify`, `result`, `done`, `error`, …) |

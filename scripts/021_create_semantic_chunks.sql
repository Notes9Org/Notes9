...
CREATE INDEX IF NOT EXISTS idx_semantic_chunks_created_by ON semantic_chunks(created_by);
...
CREATE INDEX IF NOT EXISTS idx_chunk_jobs_created_by ON chunk_jobs(created_by);
...
BEGIN
  ...
  IF TG_OP = 'DELETE' THEN
    ...
    ELSIF TG_TABLE_NAME = 'reports' THEN
      ...
      proj_id := OLD.project_id;
      exp_id := OLD.experiment_id;
      created_by_val := OLD.generated_by;
    ...
    INSERT INTO chunk_jobs (source_type, source_id, operation, created_by, payload)
    VALUES (
      source_type_value,
      OLD.id,
      'delete',
  ...
  ELSIF TG_TABLE_NAME = 'reports' THEN
    ...
    proj_id := NEW.project_id;
    exp_id := NEW.experiment_id;
    created_by_val := NEW.generated_by;
    content_text := NEW.content;
    title_text := NEW.title;
